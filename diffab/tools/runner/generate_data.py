...
def generate_data(args):
    ...
    for sabdab_entry in sabdab_entries:
        # check if it is an antigen-antibody complex, if not, skip
        if sabdab_entry["antigen_chains"] == "":
            print(str(i) + "/" + str(len_sabdab_entries))
            i = i+1
            continue
        else:
            pdb_path_ = args.pdb_data + sabdab_entry["pdb"] + ".pdb"        
            # Structure loading
            ...
            for variant in data_variants:
                ...
                save_pdb(data_native, os.path.join(log_dir, variant['tag'], sabdab_entry["pdb"] + "_REF" + ".pdb"))       # w/  OpenMM minimization
                ...
                for batch in tqdm(loader, desc=variant['name'], dynamic_ncols=True):
                    ...
                    # loop through the array of chosen id's
                    id_list = [0, 2, 5, 7, 10]
                    for id_ in id_list:
                        print(id_)
                        aa_new = traj_batch[id_][2].cuda()   # 0: Last sampling step. 2: Amino acid.
                        aa_new = aa_new.cuda()
                        pos_atom_new, mask_atom_new = reconstruct_backbone_partially(
                            pos_ctx = batch['pos_heavyatom'],
                            R_new = so3vec_to_rotation(traj_batch[id_][0].cuda() ),
                            t_new = traj_batch[id_][1].cuda(),
                            aa = aa_new,
                        ...
                        for i in range(aa_new.size(0)):
                            ...
                            save_path = os.path.join(log_dir, variant['tag'], sabdab_entry["pdb"] + "_" + str(count) + "_" + str(id_) + ".pdb")
                            save_pdb({
...
def args_factory(**kwargs):
    default_args = EasyDict(
        ...
        # light = 'L',
        no_renumber = False,
        config = './configs/test/generate_multicdrs.yml',
        out_root = './results',
        tag = '',
        seed = None,
        device = 'cuda',
...
if __name__ == '__main__':
    generate_data(args_from_cmdline())
